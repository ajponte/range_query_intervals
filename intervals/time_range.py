...
class TimeRangeInterval:
    ...
    def __init__(
        ...
        if bounds is not None and (
                IntervalBoundary.validate(
                    lower=bounds[0], upper=bounds[1]
                ) is False
        ):
            message = "bound must be one of: "
            message += f"'{IntervalBoundary.CLOSED_LEFT, IntervalBoundary.OPEN_RIGHT}', "
            message += f"'{IntervalBoundary.CLOSED_LEFT, IntervalBoundary.CLOSED_RIGHT}', "
            message += f"'{IntervalBoundary.OPEN_LEFT, IntervalBoundary.CLOSED_RIGHT}', "
